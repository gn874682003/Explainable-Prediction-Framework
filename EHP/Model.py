...
def viewResult(X_Test, Y_Test, rnn, type, data,caseid, ConvertReflact,attribute):
    for x, y,cid in zip(X_Test, Y_Test,caseid):
        ...
        for j in range(prediction.size(1)):
            yi = 0
            labels = ['Take in charge ticket','Value 1','1','Value 2','Value 3']#10.0387,11.,,7.3245
            lablen = []
            lablea = []
...
def TestMetricB(X_Test, Y_Test, rnn, type):
    ...
    if type == 1:
        for x, y in zip(X_Test, Y_Test):
            ...
            for line1, line2 in zip(y.numpy().tolist(), np.round(prediction.detach().numpy()).tolist()):#[0]
                true_y.append(line1)
                pred_y.append(line2)
        Metric = accuracy_score(true_y, pred_y)
    else:
...
def baseLine(X,Y,X_Test,Y_Test,epoch,type,input_size=2,hidden_size=32,num_layers=1,method=None, actSum=1):
    if method == 'fnn':
        n = FNN(input_size,hidden_size)
    elif method == 'rnn':
        n = RNN(input_size, hidden_size, num_layers)
    elif method == 'rnna':
        n = RNNA(input_size, hidden_size, num_layers, actSum)
    ...
    for i in range(epoch):
        ...
        if i == 0:
            ...
            BestResult = Metric
            BestModel = n
    return BestResult, BestModel#Metric, n #
...
class RNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for ir in xR:
            ...
            # print(torch.tanh(cn[1,0,:]).mean().data)#
        # print('-----------------------------')
        r_out = self.rnn(x)
        outs1 = self.out1(F.relu(r_out[0]))
        outs = self.out2(F.relu(outs1))
        return outs.view(-1, x.size(1))#[:,-1].view(-1)#
...
class RNNA(nn.Module):
    def __init__(self,input_size,hidden_size,num_layers,output_size):
        ...
        self.out1 = nn.Linear(hidden_size+input_size-1, int(hidden_size/2))#
        self.out2 = nn.Linear(int(hidden_size/2), output_size)

    def forward(self, x):
        r_out = self.rnn(x[:, :, 0:1])
        other_x = x[:, :, 1:]
        r_out = F.relu(r_out[0])
        x2 = torch.cat([r_out, other_x], dim=2)
        outs1 = self.out1(x2)#
        outs = self.out2(F.relu(outs1))
        return outs#[:,-1].view(-1)#.view(-1, x.size(1))
